...
{
 return /*(int)pow(2,index-1)*(2*colour-1)*/((index+colour)*(index+colour)+index+3*colour)/2;
}
...
void addAdjacencyConstraints(int n1, int n2)
{
  ...
  for(int i=0;i<colours;i++)
    notBothOfThese(getVar(n1,i), getVar(n2,i));
...
void addColouringConstraint(int n)
{
 ...
 minisat_Var candidate[colours];
 for(i=0;i<colours;i++){
   candidate[i]=getVar(n,i);
 }
 atLeastOneOfThese(candidate,colours);

 // Should not be two colours
 // Foreach pair of colours c1 and c2
 // ~(n is c1 and n is c2) = ~(n is c1) or ~(n is c2)
 for(i=0;i<colours-1;i++)
  for(j=i+1;j<colours;j++)
    notBothOfThese(getVar(n,i), getVar(n,j));
}
...
int checkColouring(int* colouring)
{
  ...
  for(i=1;i<graph->MaxSize;i++){
    ...
    while (p) {
      ...
      if(colouring[i]==colouring[p->index])
        return 0;
      p=p->next;
    }
  }

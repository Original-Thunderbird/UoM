...
void sort_by_ratio(){
    ...
    for(i=1;i<=Nitems;i++){
        printf("%d\t%d\t%d\t", temp_indexes[i], item_weights[temp_indexes[i]], item_values[temp_indexes[i]]);
...
int check_evaluate_and_print_sol(int *sol, int *total_value, int *total_weight)
{
  ...
  // items in pack in ascending order.
    ...
    *total_weight=0;  // total weight packed
    ...
    for(i=1;i<=Nitems;i++){
        if(sol[i]==1)   pack[temp_indexes[i]]=1;
        else            pack[temp_indexes[i]]=0;
    }
    ...
    if(*total_weight>Capacity)  return -1;  // return -1 for infeasible solutions
    else                        return 0; // return 0 for feasible solutions
}

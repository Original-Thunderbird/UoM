...
GLshort count = 0, jump_trigger = 0, jumping=0, up=1, down=0;
GLfloat t, savey,tempy;
...
void cursor_keys(int key, int x, int y) {
  switch (key) {
    ...
    case GLUT_KEY_HOME:
      ...
      break;
    case GLUT_KEY_UP:
      eyex += sin((lon+mlon)* DEG_TO_RAD) *RUN_SPEED;
      eyez += cos((lon+mlon)* DEG_TO_RAD) * RUN_SPEED;
      break;
    case GLUT_KEY_DOWN:
      eyex -= sin((lon+mlon)* DEG_TO_RAD) * RUN_SPEED;
      eyez -= cos((lon+mlon)* DEG_TO_RAD) * RUN_SPEED;

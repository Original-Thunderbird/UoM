...
GLfloat  eyex=13920*1000*8,    eyey=13920*1000*8,    eyez=13920*1000*8;    /* Eye point                */
...
void setView (void) {
  ...
  case TOP_VIEW:
    ...
    break;
  case ECLIPTIC_VIEW:
  gluLookAt(1, 0.0, bodies[0].radius*35,  // camera position
            0.0, 0.0, 0.0,    // point to look at
            0.0, 1.0, 0.0 );  // "upright" vector
    move=0;
    break;
  ...
  gluLookAt(eyex, eyey, eyez,  // camera position
    ...
    break;
  case EARTH_VIEW:
  gluLookAt(bodies[3].orbital_radius*cos(bodies[3].orbit*DEG_TO_RAD),bodies[3].radius*1.12,bodies[3].orbital_radius*sin(bodies[3].orbit*DEG_TO_RAD),  // camera position
            0.0, 0.0, 0.0,    // point to look at
            0.0, 1.0, 0.0 );  // "upright" vector
    move=0;
    break;
...
void drawBody(int n){
  //if(n==0 || n==1){
    ...
    glRotatef(bodies[n].spin,0,1,0);//spin aournd body's axis

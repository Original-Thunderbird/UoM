...
public class BasicInvertedIndex extends Configured implements Tool
{
    ...
    public static class Map extends 
            Mapper<Object, Text, Text, ArrayListWritable<Writable>>
    {
                ...
                throws IOException, InterruptedException
        {
            ...
            while (itr.hasMoreTokens())
            {
            	...
            	if(raw.contains("http"))//preserve format of web
            		raw = raw.replaceAll("\"", "");
            	...
            	if(Pattern.matches("^\\d+$|^.$|(^[0-9][a-z][0-9][0-9]$)", raw)) continue;
            	//stopword removal
            	if(stopAnalyser.isStopWord(raw))	continue;
            	//stemmer
            	...
            	if(cache.containsKey(finkey)==false) {//new token, initialise array of index, insert
...
//        		context.write(TOKEN, indexlist);
    ...
    public static class Reduce extends Reducer<Text, ArrayListWritable<Writable>, Text, HashMapWritable<Text,ArrayListWritable<Writable>>>
    {
        ...
        public void reduce(
                Text key,
                Iterable<ArrayListWritable<Writable>> values,
                Context context) throws IOException, InterruptedException
        {
            ...
            while (iter.hasNext()) {
            	...
            	tmp.remove(0);
            	//put in local cache
            	if(cache.containsKey(doc)==false) {//new token, insert
            		cache.put(doc, tmp);
            	}
            	else {//old token, replace
